# CKKS bootstrapping, S2C-first — the AVX2 / FMA submission

The harness as `fherma implementation init ckks-bootstrapping/s2c@1.0.0` writes
it, with two lines changed: `Cargo.toml` selects the `avx` backend
(AVX2 with FMA, single-threaded) and `config.jsonc` asks for 1 thread.
`src/run.rs` is the reference's, untouched: this row competes on backend and
threads, not on the algorithm.

Poulpy publishes 13.3 s for this backend on a Ryzen 9 9950X. The number here
is this backend on the platform's machine, which is a different processor — the
rows of this specification compare with each other, not with somebody else's
bench.
...
Locally nothing judges the digest, and a digest made on another operating
system will not equal the platform's: Poulpy encodes the bootstrapping
matrices through `f64` and libm, and Apple's libm and glibc round differently
(seed 1 at this point: `1818321b…` on macOS, `0b2a6ae1…` on the platform's
image). The platform judges on its own machines, all running the same image,
so what counts is what it measures. Run locally to see that it builds, runs
and reaches the precision; leave the equality to the platform.
